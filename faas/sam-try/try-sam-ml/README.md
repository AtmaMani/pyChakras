...
```bash
try-sam-ml$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
try-sam-ml$ sam local start-api
try-sam-ml$ curl http://localhost:3000/classify_digit
```
...
```bash
try-sam-ml$ sam logs -n InferenceFunction --stack-name try-sam-ml --tail
```
...
```bash
aws cloudformation delete-stack --stack-name try-sam-ml
```
